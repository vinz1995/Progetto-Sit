...
//	caricato in OpenJUMP 
	...
	public void AF1(PlugInContext context) {
...
//		Layer layer_selezionato=context.getLayerManager().getLayer("Nuovo");
	...
	public void Ese64Bonus(PlugInContext context) {
		...
		for (Feature f : layer.getFeatureCollectionWrapper().getFeatures()){
			// System.out.println("numero di feature: "+layer.getFeatureCollectionWrapper().getFeatures().size());
			// System.out.println("id: "+ f.getAttribute("id"));
			// System.out.println("testo: "+ f.getAttribute("testo"));
			// System.out.println("geometry: "+ f.getGeometry());
			String query="insert into test (id,testo,geometry) values ("+f.getAttribute("id")+",'"+f.getAttribute("testo")+"',st_GeomFromText('"+f.getGeometry()+"'))";
